...
CImg<unsigned char> ope_dilatation(CImg<unsigned char> &I, int size,double alpha, float h,double s){
  ...
  for (int i = 0; i < n; i++){
	...
	for(int kj = -se; kj <= se; kj++){
	  if(ki == 0 && kj == 0){
	    continue; // si on est au centre c'est a dire a (i,j) on ignore le pixel
	  }
	  else{
	    ...
	    if( w > -s){
	      ...
	      if(padding(i+ki+padd,j+kj+padd) > max){
		max = padding(i+ki+padd,j+kj+padd); // on le remplace
	      }
	    }
	  }
	}
	...
	for(int kj = -se; kj <= se; kj++){
	  ...
	  if(ki == 0 && kj == 0){
	    ...
	    if( w > -s){
	      ...
	      if(zero(i+ki+padd,j+kj+padd) < min){
		min = zero(i+ki+padd,j+kj+padd); // on le remplace
	      }
	    }
	  }
	}
...
CImg<unsigned char> gammaPhi(CImg<unsigned char> I,int patchSize, float h, double alpha, double s){
...
CImg<unsigned char> phiGamma(CImg<unsigned char> I,int patchSize, float h, double alpha, double s){
...
int main(int argc, char **argv){
  ...
  CImg<unsigned char> dila = ope_dilatation(noisi,patchSize,alpha,h,s);
  CImg<unsigned char> ero = ope_erosion(noisi,patchSize,alpha,h,s);
  CImg<unsigned char> gamma = gammaPhi(original,patchSize,alpha,h,s);
  CImg<unsigned char> phi = phiGamma(original,patchSize,alpha,h,s);
  cout << "calcul final" << endl;
  CImg<unsigned char> fin = ouv_ferm(gamma,phi);
  CImgList<unsigned char> main_visu(original,noisi,dila,ero,gamma,phi,fin);
  ...
  cout << "ouverture : " << gamma.MSE(original) << endl;
  cout << "fermeture : " << phi.MSE(original) << endl;
  ...
  cout << "DB ouverture : " << gamma.PSNR(original,255) << endl;
  cout << "DB fermeture : " << phi.PSNR(original,255) << endl;
